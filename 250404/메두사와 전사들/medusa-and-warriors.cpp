...
pii warrior[MAX_M];
int sightMap[MAX_N][MAX_N];
int warrior_cnt[MAX_N][MAX_N];

int dx[4] = {-1, 1, 0, 0};
int dy[4] = {0, 0, -1, 1};

inline int cal_mangattan(const pii& a, const pii& b) {
...
int main() {
    ...
    if (dist[startX][startY] == -1) {
        cout << "-1\n";
        return 0;
    }

    int curX = startX;  // 현재 플레이어의 X좌표
    int curY = startY;  // 현재 플레이어의 Y좌표
